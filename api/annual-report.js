/**
 * Vercel serverless function to generate and post annual trend reports
 * This function is triggered by a cron job
 */

const { generateAnnualReport, formatTrendReportForNostr, formatTrendReportForTelegram } = require('../lib/trendAnalysis');
...
export default async function handler(req, res) {
  try {
    ...
    console.log(`StrichBot v${versionInfo.fullVersion}: Starting annual report generation (${authStatus}, IP: ${securityCheck.clientIp})`);
    ...
    const annualReportEnabled = process.env.ANNUAL_REPORT_ENABLED !== 'false';
    const nostrEnabled = process.env.ANNUAL_REPORT_NOSTR !== 'false';
    const telegramEnabled = process.env.ANNUAL_REPORT_TELEGRAM !== 'false';

    if (!annualReportEnabled) {
      console.log('Annual reports are disabled');
      return res.status(200).json({
        success: true,
        message: 'Annual reports are disabled',
        timestamp: new Date().toISOString()
      });
    }

    console.log('Annual report configuration:', {
      nostrEnabled,
      telegramEnabled,
    ...
    if (!report.success) {
      console.error('Failed to generate annual report:', report.error);
      return res.status(500).json({
        success: false,
        error: `Failed to generate annual report: ${report.error}`,
        timestamp: new Date().toISOString()
      });
    }
    ...
    if (nostrEnabled && nsec && relayString) {
      try {
        console.log('Posting annual report to Nostr...');
        ...
        console.log('Annual report posted to Nostr successfully:', {
        ...
        console.error('Failed to post annual report to Nostr:', nostrError);
        results.nostr = {
          enabled: true,
          sent: false,
      ...
      console.log('Nostr posting disabled or not configured for annual reports');
    ...
    if (telegramEnabled && telegramBotToken && telegramChatId) {
      try {
        console.log('Posting annual report to Telegram...');
        ...
        console.log('Annual report posted to Telegram successfully:', {
        ...
        console.error('Failed to post annual report to Telegram:', telegramError);
        results.telegram = {
          enabled: true,
          sent: false,
      ...
      console.log('Telegram posting disabled or not configured for annual reports');
    ...
    const message = overallSuccess
      ? 'Annual report generated and posted successfully'
      : 'Annual report generated but some posting failed';
    ...
    console.error('Error in annual report generation:', error);
