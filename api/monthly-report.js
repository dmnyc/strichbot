/**
 * Vercel serverless function to generate and post monthly trend reports
 * This function is triggered by a cron job
 */

const { generateMonthlyReport, formatTrendReportForNostr, formatTrendReportForTelegram } = require('../lib/trendAnalysis');
...
export default async function handler(req, res) {
  try {
    ...
    console.log(`StrichBot v${versionInfo.fullVersion}: Starting monthly report generation (${authStatus}, IP: ${securityCheck.clientIp})`);
    ...
    const monthlyReportEnabled = process.env.MONTHLY_REPORT_ENABLED !== 'false';
    const nostrEnabled = process.env.MONTHLY_REPORT_NOSTR !== 'false';
    const telegramEnabled = process.env.MONTHLY_REPORT_TELEGRAM !== 'false';

    if (!monthlyReportEnabled) {
      console.log('Monthly reports are disabled');
      return res.status(200).json({
        success: true,
        message: 'Monthly reports are disabled',
        timestamp: new Date().toISOString()
      });
    }

    console.log('Monthly report configuration:', {
      nostrEnabled,
      telegramEnabled,
    ...
    if (!report.success) {
      console.error('Failed to generate monthly report:', report.error);
      return res.status(500).json({
        success: false,
        error: `Failed to generate monthly report: ${report.error}`,
        timestamp: new Date().toISOString()
      });
    }
    ...
    if (nostrEnabled && nsec && relayString) {
      try {
        console.log('Posting monthly report to Nostr...');
        ...
        console.log('Monthly report posted to Nostr successfully:', {
        ...
        console.error('Failed to post monthly report to Nostr:', nostrError);
        results.nostr = {
          enabled: true,
          sent: false,
      ...
      console.log('Nostr posting disabled or not configured for monthly reports');
    ...
    if (telegramEnabled && telegramBotToken && telegramChatId) {
      try {
        console.log('Posting monthly report to Telegram...');
        ...
        console.log('Monthly report posted to Telegram successfully:', {
        ...
        console.error('Failed to post monthly report to Telegram:', telegramError);
        results.telegram = {
          enabled: true,
          sent: false,
      ...
      console.log('Telegram posting disabled or not configured for monthly reports');
    ...
    const message = overallSuccess
      ? 'Monthly report generated and posted successfully'
      : 'Monthly report generated but some posting failed';
