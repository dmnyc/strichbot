...
async function formatWeeklyStatsMessage(stats, blockData = null) {
  ...
  try {
    ...
    if (weekAgoStats) {
      ...
      const formattedMemberPercent = memberPercentChange >= 0 ? `+${memberPercentChange.toFixed(1)}` : `${memberPercentChange.toFixed(1)}`;
      const formattedChannelPercent = channelPercentChange >= 0 ? `+${channelPercentChange.toFixed(1)}` : `${channelPercentChange.toFixed(1)}`;
      const formattedCapacityPercent = capacityPercentChange >= 0 ? `+${capacityPercentChange.toFixed(1)}` : `${capacityPercentChange.toFixed(1)}`;

      trendSection = `
👥 Members: ${formattedMembers}
   7-day change: ${formattedMemberChange} (${formattedMemberPercent}%) ${memberIndicator}

🔗 Channels: ${formattedChannels}
   7-day change: ${formattedChannelChange} (${formattedChannelPercent}%) ${channelIndicator}

🪙 Capacity: ${capacityBTC} BTC
   7-day change: ${formattedCapacityChange} BTC (${formattedCapacityPercent}%) ${capacityIndicator}`;
    } else {
