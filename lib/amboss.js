...
async function fetchNodestrichStats(apiKey = null) {
  try {
    ...
    // Based on research, we'll query for general network stats and specific community data
    const query = `
    ...
    const stats = data.data?.getStats?.network;

    if (!stats) {
      throw new Error('No network statistics found in response');
    }

    return {
      memberCount: stats.totalNodes || 0,
      totalChannels: stats.totalChannels || 0,
      totalCapacity: stats.totalCapacity || 0,
      avgChannelSize: stats.avgChannelSize || 0,
