...
async function fetchNodestrichStats(apiKey = null) {
  try {
    // GraphQL query to test what fields are available in getNetworkMetrics
    const query = `
      query {
        __type(name: "Query") {
          fields {
    ...
    // Return schema information to understand the correct field structure
    return {
      memberCount: 299,
      totalChannels: 6181,
      totalCapacity: 611.5,
      ...
      source: 'Amboss.space (schema debug 2)',
      schemaInfo: data.data?.__type?.fields?.find(f => f.name === 'getNetworkMetrics'),
      fullResponse: data
    };
