...
async function fetchNodestrichStats(apiKey = null) {
  try {
    ...
    // Extract the actual network metrics
    const metrics = data.data?.getNetworkMetrics;

    if (!metrics) {
      throw new Error('No network metrics found in response');
    }

    return {
      memberCount: metrics.totalNodes || 0,
      totalChannels: metrics.totalChannels || 0,
      totalCapacity: metrics.totalCapacity ? (metrics.totalCapacity / 100000000).toFixed(2) : 0, // Convert sats to BTC
      avgChannelSize: metrics.avgChannelSize || 0,
      timestamp: new Date().toISOString(),
      source: 'Amboss.space',
      rawMetrics: metrics // Include raw data for debugging
    };
