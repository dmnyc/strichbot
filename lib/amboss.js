...
async function fetchNodestrichStats(apiKey = null) {
  try {
    // Try to get Nodestrich community data specifically
    const query = `
      query {
        getCommunity(name: "nodestrich") {
          name
          memberCount
          description
        }
    ...
    // Test if community query works
    const community = data.data?.getCommunity;

    return {
      memberCount: community?.memberCount || 299,
      totalChannels: 6181, // Will need separate query for this
      totalCapacity: 611.5, // Will need separate query for this
      avgChannelSize: 0.099,
      timestamp: new Date().toISOString(),
      source: community ? 'Amboss.space (community)' : 'Amboss.space (fallback)',
