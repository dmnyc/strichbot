...
async function fetchNodestrichStats(apiKey = null) {
  try {
    // GraphQL query to test what getNetworkMetrics actually returns
    const query = `
      query {
        getNetworkMetrics
      }
    ...
    // See what getNetworkMetrics actually returns
    const networkMetrics = data.data?.getNetworkMetrics;
    ...
    return {
      ...
      source: 'Amboss.space (testing metrics)',
      networkMetrics: networkMetrics,
      networkMetricsType: typeof networkMetrics,
      fullResponse: data
    };
